...
const AuthContextProvider = ({ children }) => {
  ...
  const passwordReset = async(formData) => {
    try{
      const res = await axios.post(`${API}/accounts/api/password_reset/`,formData)
  ...
  const confirmPasswordReset = async(formData)=>{
    try{
      const res = await axios.post(`${API}/accounts/api/password_reset/confirm/`,formData)
    }
    catch(error){
      console.log(error)
      setError([error.response.data.detail])
  ...
  // console.log(error)
  return (
    <authContext.Provider value={{ register, error ,setError,login,passwordReset,confirmPasswordReset}}>
      {children}
